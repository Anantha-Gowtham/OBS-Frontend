...
const UserDashboard = () => {
  ...
  const [loading, setLoading] = useState(false); // future: set true while fetching

  // Example effect placeholder for data fetch
  useEffect(() => {
    // if implementing fetch: setLoading(true); fetch(...).finally(()=>setLoading(false));
  }, []);
  ...
  const bankingStyles = {
    ...
    statIconPrimary: (theme) => ({
      backgroundColor: `${theme.palette.primary.main}10`,
      color: theme.palette.primary.main,
    }),
    statIconSuccess: (theme) => ({
      backgroundColor: `${theme.palette.success.main}10`,
      color: theme.palette.success.main,
    }),
    statIconWarning: (theme) => ({
  ...
  // Animated totals using count-up hook
  const animatedBalance = useCountUp(getTotalBalance(), 900, 0);
  const animatedIncome = useCountUp(42500, 900, 0);
  const animatedExpenses = useCountUp(23800, 900, 0);
  const animatedSavings = useCountUp(387500, 900, 0);

  if (loading) {
    return (
      <Box>
        <Container maxWidth="xl" sx={{ py: 3 }}>
          <SkeletonBlock height={40} width="30%" sx={{ mb: 3 }} />
          <Grid container spacing={3} sx={{ mb: 4 }}>
            {Array.from({ length: 4 }).map((_, i) => (
              <Grid size={{ xs: 12, sm: 6, md: 3 }} key={i}>
                <Card sx={{ p: 2 }}>
                  <SkeletonBlock height={22} width="70%" sx={{ mb: 1 }} />
                  <SkeletonBlock height={32} width="60%" sx={{ mb: 1 }} />
          ...
          <Grid container spacing={3}>
            <Grid size={{ xs: 12, md: 8 }}>
              <Card sx={{ p: 2, mb: 3 }}>
                <SkeletonBlock height={24} width="50%" sx={{ mb: 2 }} />
                {Array.from({ length: 5 }).map((_, r) => (
                  <SkeletonBlock key={r} height={18} width={`${95 - r * 7}%`} sx={{ mb: 1 }} />
                ))}
              </Card>
            </Grid>
            <Grid size={{ xs: 12, md: 4 }}>
              <Card sx={{ p: 2, mb: 3 }}>
                <SkeletonBlock height={24} width="60%" sx={{ mb: 2 }} />
  ...
  return (
    <Box>
      <Container maxWidth="xl" sx={{ py: 3 }}>
        ...
        <Grid container spacing={3} sx={{ mb: 4 }} data-reveal>
          {/* Account Summary */}
          <Grid size={{ xs: 12, md: 8 }} className="fade-up">
            <Card sx={bankingStyles.dashboardCard}>
              <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
                Account Summary
              </Typography>
              <Grid container spacing={3}>
                <Grid size={{ xs: 12, sm: 6 }} data-reveal="delay-1" className="fade-up">
                  <Box sx={bankingStyles.statCard}>
                    <Box sx={{ ...bankingStyles.statIcon, ...bankingStyles.statIconPrimary }}>
                      <Wallet />
                    </Box>
                    <Box>
                      <Typography sx={bankingStyles.statValue} className="fade-up" style={{ animationDelay: '.1s' }}>
                ...
                <Grid size={{ xs: 12, sm: 6 }} data-reveal="delay-2" className="fade-up">
                  <Box sx={bankingStyles.statCard}>
                    <Box sx={{ ...bankingStyles.statIcon, ...bankingStyles.statIconSuccess }}>
                      <ArrowUpward />
                    </Box>
                    <Box>
                      <Typography sx={bankingStyles.statValue} style={{ animation: 'fadeUp .8s ease both', animationDelay: '.15s' }}>
                        ₹{Math.round(animatedIncome).toLocaleString()}
                      </Typography>
                      <Typography sx={bankingStyles.statLabel}>
                        Monthly Income
                      </Typography>
                ...
                <Grid size={{ xs: 12, sm: 6 }} data-reveal="delay-3" className="fade-up">
                  <Box sx={bankingStyles.statCard}>
                    <Box sx={{ ...bankingStyles.statIcon, ...bankingStyles.statIconWarning }}>
                      <ArrowDownward />
                    </Box>
                    <Box>
                      <Typography sx={bankingStyles.statValue} style={{ animation: 'fadeUp .8s ease both', animationDelay: '.2s' }}>
                        ₹{Math.round(animatedExpenses).toLocaleString()}
                      </Typography>
                      <Typography sx={bankingStyles.statLabel}>
                        Monthly Expenses
                      </Typography>
                ...
                <Grid size={{ xs: 12, sm: 6 }} data-reveal="delay-4" className="fade-up">
                  <Box sx={bankingStyles.statCard}>
                    <Box sx={{ ...bankingStyles.statIcon, ...bankingStyles.statIconInfo }}>
                      <Savings />
                    </Box>
                    <Box>
                      <Typography sx={bankingStyles.statValue} style={{ animation: 'fadeUp .8s ease both', animationDelay: '.25s' }}>
          ...
          {/* Quick Actions */}
          <Grid size={{ xs: 12, md: 4 }} className="fade-up" data-reveal="delay-2">
            <Card sx={bankingStyles.dashboardCard}>
              <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
                Quick Actions
              </Typography>
        ...
        <Grid container spacing={3}>
          {/* Recent Transactions */}
          <Grid size={{ xs: 12, md: 8 }} className="fade-up" data-reveal>
            <Card sx={bankingStyles.dashboardCard}>
          ...
          {/* Account Security */}
          <Grid size={{ xs: 12, md: 4 }} className="fade-up" data-reveal="delay-1">
            <Card sx={bankingStyles.dashboardCard}>
              <Typography variant="h5" sx={{ mb: 3, fontWeight: 600 }}>
                Account Security
              </Typography>
